 ...
 * 
 * Modified to use mask.bat file for windows
 * 
 * @author Sandeep Gupta (sandy.pec@gmail.com)
 */
public class Main extends TestSuite {

	private static final String MASK_SH = "mask.bat";

	private static int testsPassed = 0;

	public static void main(String[] args) throws IOException {
		if (!new File(MASK_SH).exists()) {
			System.err
					.println("Couldn't find 'mask.sh' in the current directory.");
			System.exit(1);
		}
		...
		final Process process = new ProcessBuilder(MASK_SH).start();
		...
		try {
			...
			for (int i = 0; i < iterations; i++) {
				...
				// Write in the background. Writing can block if the buffer
				// fills up.
				executor.execute(new Runnable() {
					public void run() {
						try {
				...
				luhnyBinTests.check(in, new TestCase.Listener() {
					...
					public void testFailed(TestCase test, String actualInput) {
						...
						System.out.println();
						System.err.println("Test #" + test.index + " of "
								+ luhnyBinTests.count + " failed:"
								+ "\n  Description:     " + test.description
								+ "\n  Input:           "
								+ showBreaks(test.output)
								+ "\n  Expected result: "
								+ showBreaks(test.expectedInput)
								+ "\n  Actual result:   "
								+ showBreaks(actualInput) + "\n");
						process.destroy();
						System.exit(1);
					}
				});
			...
			if (iterations > 1) {
				...
				System.out.printf("Median time:  %,dus%n",
			...
			System.err
					.println("Error: mask.sh didn't send the expected amount of output.");
			process.destroy();
			System.exit(1);
		}
	}
